...
var_c = [1500, 1000, 1000, 1000, 30, 30, 0.1];
...
%compute the error bewteen estimated and harr
    number = size(bbox,1);
    error(1,index) = index;
    %error(2,index) = sqrt(min(diag((bbox - repmat(state_clr,number,1)) * (bbox - repmat(state_clr,number,1))')));
    index = index + 1;
...
title('dynamic error change of color-based tracker');
xlabel('time');
ylabel('sqrt error');
