...
%         rectangle('Position', BB_clr, 'EdgeColor', 'r', 'LineWidth', 2);
    ...
    % calculate histogram for each particle
    for i = 1 : N
...
%     w_clr = weight_clr(hist_t, hist_p, sigma_c);
   ...
   % rectangle('Position', state_clr, 'EdgeColor', 'b', 'LineWidth', 2.5);
    ...
    number = size(bbox,1);
    error(1,index) = index;
    error(2,index) = sqrt(min(diag((bbox - repmat(state_mnt,number,1)) * (bbox - repmat(state_mnt,number,1))')));
    index = index + 1;
...
title('dynamic error change of moment-based tracker');
xlabel('time');
ylabel('sqrt error');
